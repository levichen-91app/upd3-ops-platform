...
export class HttpExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(HttpExceptionFilter.name);
    ...
    if (exception instanceof HttpException) {
      ...
      // Check if exception response has custom error structure (includes complete error objects)
      if (
        ...
        'code' in exceptionResponse.error
      ) {
        const customError = exceptionResponse.error as any;
        errorCode = customError.code;
        message = customError.message;
        details = customError.details;
      }
      // Check if exception response has simple custom error structure
      else if (
        typeof exceptionResponse === 'object' &&
        exceptionResponse !== null &&
      ...
      // Handle authorization errors (only as fallback when no custom structure is found)
      else if (status === 401) {

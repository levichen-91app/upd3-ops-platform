  ...
  async getNotificationDetail(
    ...
    try {
      const response = await firstValueFrom(
        this.httpService
          .get<NcDetailApiResponse>(url, {
            timeout: this.config.timeout,
          })
          .pipe(
            catchError((error: AxiosError) => {
              this.logger.error(
                `NC Detail API error: ${error.message}`,
                error.stack,
              );

              if (error.code === 'ECONNABORTED') {
                throw new Error(
                  `TIMEOUT_ERROR: NC Detail API timeout after ${this.config.timeout}ms`,
                );
              }
              ...
              if (error.response) {
                ...
                if (status === 400) {
                  throw new Error(
                    `VALIDATION_ERROR: ${JSON.stringify(errorData)}`,
                  );
                }

                if (status >= 500) {
                  throw new Error(
                    `EXTERNAL_API_ERROR: NC Detail API returned ${status}`,
                  );
                }
              }
      ...
      if (!response.data || typeof response.data !== 'object') {
        throw new Error(
          'DATA_FORMAT_ERROR: Invalid response format from NC Detail API',
        );
      }
      ...
      if (apiResponse.StatusCode !== 200) {
        throw new Error(
          `EXTERNAL_API_ERROR: NC Detail API returned status ${apiResponse.StatusCode}`,
        );
      }
      ...
      this.logger.error(
        `Failed to get notification detail for shopId: ${shopId}, ncId: ${ncId}`,
        error,
      );
      throw error;
    }
